...
import com.imguns.guns.client.model.functional.*;
import com.imguns.guns.client.model.listener.model.ModelAdditionalMagazineListener;
import com.imguns.guns.client.resource.index.ClientAttachmentIndex;
import com.imguns.guns.client.resource.pojo.display.gun.TextShow;
...
import static com.imguns.guns.client.model.GunModelConstant.*;
...
public class BedrockGunModel extends BedrockAnimatedModel {
    ...
    public BedrockGunModel(BedrockModelPOJO pojo, BedrockVersion version) {
        ...
        this.magazineNode = Optional.ofNullable(modelMap.get(MAG_NORMAL_NODE)).map(ModelRendererWrapper::modelRenderer).orElse(null);
        this.additionalMagazineNode = Optional.ofNullable(modelMap.get(MAG_ADDITIONAL_NODE)).map(ModelRendererWrapper::modelRenderer).orElse(null);
    ...
    private void cacheOtherPath() {
        ...
        root = Optional.ofNullable(modelMap.get(ROOT_NODE)).map(ModelRendererWrapper::modelRenderer).orElse(null);
    }
    ...
    private IFunctionalRenderer renderAdditionalMagazine(BedrockPart bedrockPart) {
        return (poseStack, vertexBuffer, transformType, light, overlay) -> {
            if (bedrockPart.visible) {
                bedrockPart.compile(poseStack.peek(), vertexBuffer, light, overlay, 1.0F, 1.0F, 1.0F, 1.0F);
                ...
                if (magazineNode != null && magazineNode.visible) {
                    magazineNode.compile(poseStack.peek(), vertexBuffer, light, overlay, 1.0F, 1.0F, 1.0F, 1.0F);
    ...
    public void render(MatrixStack matrixStack, ItemStack gunItem, ModelTransformationMode transformType, RenderLayer renderType, int light, int overlay) {
