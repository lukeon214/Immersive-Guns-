...
public class ClientAttachmentItemTooltip implements TooltipComponent {
    ...
    private final MutableText tips = Text.translatable("tooltip.immersive_guns.attachment.yaw.shift");
    private final MutableText support = Text.translatable("tooltip.immersive_guns.attachment.yaw.support");
        ...
        TimelessAPI.getClientAttachmentIndex(attachmentId).ifPresent(index -> {
            ...
            if (type == AttachmentType.SCOPE) {
                ...
                if (zoom != null) {
                    ...
                    components.add(Text.translatable("tooltip.immersive_guns.attachment.zoom", zoomJoinText).formatted(Formatting.GOLD));
                }
            }
            ...
            if (type == AttachmentType.EXTENDED_MAG) {
                ...
                if (magLevel == 1) {
                    components.add(Text.translatable("tooltip.immersive_guns.attachment.extended_mag_level_1").formatted(Formatting.GRAY));
                } else if (magLevel == 2) {
                    components.add(Text.translatable("tooltip.immersive_guns.attachment.extended_mag_level_2").formatted(Formatting.BLUE));
                } else if (magLevel == 3) {
                    components.add(Text.translatable("tooltip.immersive_guns.attachment.extended_mag_level_3").formatted(Formatting.LIGHT_PURPLE));
                }
            }

            float adsAddendTime = data.getAdsAddendTime();
            if (adsAddendTime > 0) {
                components.add(Text.translatable("tooltip.immersive_guns.attachment.ads.increase").formatted(Formatting.RED));
            } else if (adsAddendTime < 0) {
                components.add(Text.translatable("tooltip.immersive_guns.attachment.ads.decrease").formatted(Formatting.GREEN));
            }

            float inaccuracyAddend = data.getInaccuracyAddend();
            if (inaccuracyAddend > 0) {
                components.add(Text.translatable("tooltip.immersive_guns.attachment.inaccuracy.increase").formatted(Formatting.RED));
            ...
            if (recoilModifier != null) {
                ...
                if (pitch > 0) {
                    components.add(Text.translatable("tooltip.immersive_guns.attachment.pitch.increase").formatted(Formatting.RED));
                } else if (pitch < 0) {
                    components.add(Text.translatable("tooltip.immersive_guns.attachment.pitch.decrease").formatted(Formatting.GREEN));
                }

                float yaw = recoilModifier.getYaw();
                if (yaw > 0) {
                    components.add(Text.translatable("tooltip.immersive_guns.attachment.yaw.increase").formatted(Formatting.RED));
                } else if (yaw < 0) {
                    components.add(Text.translatable("tooltip.immersive_guns.attachment.yaw.decrease").formatted(Formatting.GREEN));
                }
            }
        });
    }
