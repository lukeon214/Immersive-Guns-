    ...
    private void getText() {
        ...
        if (shouldShow(GunTooltipPart.BASE_INFO)) {
            ...
            if (level >= iGun.getMaxLevel()) {
                ...
                this.levelInfo = Text.translatable("tooltip.immersive_guns.gun.level").append(Text.literal(levelText).formatted(Formatting.DARK_PURPLE));
            } else {
                String levelText = String.format("%d (%.1f%%)", level, expCurrentLevel / (expToNextLevel + expCurrentLevel) * 100f);
                this.levelInfo = Text.translatable("tooltip.immersive_guns.gun.level").append(Text.literal(levelText).formatted(Formatting.YELLOW));
            }
            this.maxWidth = Math.max(font.getWidth(this.levelInfo), this.maxWidth);

            String tabKey = "imguns.type." + gunIndex.getType() + ".name";
            this.gunType = Text.translatable("tooltip.immersive_guns.gun.type").append(Text.translatable(tabKey).formatted(Formatting.AQUA));
            ...
            if (gunIndex.getBulletData().getExplosionData() != null) {
                value.append(" + ").append(DAMAGE_FORMAT.format(gunIndex.getBulletData().getExplosionData().getDamage() * SyncConfig.DAMAGE_BASE_MULTIPLIER.get())).append(Text.translatable("tooltip.immersive_guns.gun.explosion"));
            }
            this.damage = Text.translatable("tooltip.immersive_guns.gun.damage").append(value);
        ...
        if (shouldShow(GunTooltipPart.EXTRA_DAMAGE_INFO)) {
            ...
            if (extraDamage != null) {
                ...
                this.armorIgnore = Text.translatable("tooltip.immersive_guns.gun.armor_ignore", FORMAT.format(armorDamagePercent));
                this.headShotMultiplier = Text.translatable("tooltip.immersive_guns.gun.head_shot_multiplier", FORMAT.format(headShotMultiplierPercent));
            } else {
                this.armorIgnore = Text.translatable("tooltip.immersive_guns.gun.armor_ignore", FORMAT.format(0));
                this.headShotMultiplier = Text.translatable("tooltip.immersive_guns.gun.head_shot_multiplier", FORMAT.format(1));
            }
        ...
        if (shouldShow(GunTooltipPart.UPGRADES_TIP)) {
            ...
            this.tips = Text.translatable("tooltip.immersive_guns.gun.tips", keyName).formatted(Formatting.YELLOW).formatted(Formatting.ITALIC);
