...
import static com.imguns.guns.client.animation.statemachine.GunAnimationConstant.*;
...
public class GunAnimationStateMachine {
    ...
    public void onBayonetAttack(int count) {
        String animationName = "melee_bayonet_" + MathHelper.clamp(count + 1, 1, 3);
    ...
    public void update(float partialTicks, Entity entity) {
        ...
        if (runner != null) {
            ...
            float deltaDistanceWalked = entity.horizontalSpeed - entity.prevHorizontalSpeed;
            float distanceWalked;
            if (pauseWalkAndRun) {
                // 保持 distanceWalked 与 keepDistanceWalked 相同，即不随时间增长
                distanceWalked = keepDistanceWalked;
                baseDistanceWalked = entity.horizontalSpeed + deltaDistanceWalked * partialTicks - keepDistanceWalked;
            } else {
                // distanceWalked 与 keepDistanceWalked 一同随时间增长
                distanceWalked = entity.horizontalSpeed + deltaDistanceWalked * partialTicks - baseDistanceWalked;
    ...
    public boolean isPlayingAnimation(int track, @NotNull String... names) {
