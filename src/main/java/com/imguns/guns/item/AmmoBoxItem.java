...
import com.imguns.guns.api.item.nbt.AmmoBoxItemDataAccessor;
...
public class AmmoBoxItem extends Item implements DyeableItem, AmmoBoxItemDataAccessor {
    ...
    public Text getName(ItemStack stack) {
        if (isAllTypeCreative(stack)) {
            return Text.translatable("item.immersive_guns.ammo_box.all_type_creative").formatted(Formatting.DARK_PURPLE);
        }
        if (isCreative(stack)) {
            return Text.translatable("item.immersive_guns.ammo_box.creative").formatted(Formatting.DARK_PURPLE);
        }
        int ammoLevel = getAmmoLevel(stack);
        switch (ammoLevel) {
            case GOLD_LEVEL -> {
                return Text.translatable("item.immersive_guns.ammo_box.gold").formatted(Formatting.YELLOW);
            }
            case DIAMOND_LEVEL -> {
                return Text.translatable("item.immersive_guns.ammo_box.diamond").formatted(Formatting.AQUA);
            }
            default -> {
                return Text.translatable("item.immersive_guns.ammo_box.iron");
            }
        }
    }
    ...
    public void appendTooltip(ItemStack stack, @Nullable World pLevel, List<Text> components, TooltipContext isAdvanced) {
        if (isAllTypeCreative(stack)) {
            components.add(Text.translatable("tooltip.immersive_guns.ammo_box.usage.all_type_creative").formatted(Formatting.GOLD));
            return;
        }
        if (isCreative(stack)) {
            components.add(Text.translatable("tooltip.immersive_guns.ammo_box.usage.creative.1").formatted(Formatting.YELLOW));
            components.add(Text.translatable("tooltip.immersive_guns.ammo_box.usage.creative.2").formatted(Formatting.YELLOW));
            return;
        }
        components.add(Text.translatable("tooltip.immersive_guns.ammo_box.usage.deposit").formatted(Formatting.GRAY));
        components.add(Text.translatable("tooltip.immersive_guns.ammo_box.usage.remove").formatted(Formatting.GRAY));
    }
}
